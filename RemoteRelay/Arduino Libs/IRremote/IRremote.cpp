...
void IRsend::attachResult(decode_results* result)
{
  result->rawbuf = irparams.rawbuf;
  result->rawlen = irparams.rawlen;
  result->rawSign = irparams.rawSign;
}

void IRsend::sendRawDirectly(decode_results* result, int hz, bool sendgap)
{
  enableIROut(hz);
  if (result->rawlen == 0)
  {
  ...
  if (result->rawlen > RAWBUF)
  {
    return;
  }
  for (int i = 0; i < result->rawlen; i++) {
    if ((i & 1) == 0) {
      mark(((unsigned int)(result->rawbuf[i]))*USECPERTICK);
    } 
    else {      
      space(((unsigned int)(result->rawbuf[i]))*USECPERTICK);
    }
  }
  if(sendgap)
    space(result->rawSign*USECPERTICK);
  ...
  case STATE_IDLE: // In the middle of a gap
    if (irdata == MARK) {
      if (irparams.timer < GAP_TICKS) {
        ...
        irparams.rawlen = 0;
        irparams.rawSign = irparams.timer;
        irparams.timer = 0;
...
int IRrecv::decode(decode_results *results) {
  ...
  results->rawSign = irparams.rawSign;
